...
function menorFaturamento(dados) {
  const valoresFaturamento = dados.map(dado => dado.valor).filter(valor => valor > 0);
...
function maiorFaturamento(dados) {
  ...
  const diasFaturados = dados.filter((dia) => dia.valor > 0);

  const valorMaximo = diasFaturados.reduce((prev, current) => {
...
function diasAcimaDaMedia(dados) {
  const somaFaturamento = dados.reduce((acc, dia) => acc + dia.valor, 0);
  ...
  console.log(`Número de dias em que o valor de faturamento diário foi superior à média mensal: ${count}`);
}
